...
lasso.optimal.s <- lapply(lasso.models, function(lasso.model) {
    cp.diff <- 0:(length(lasso.model$Cp)-1) - lasso.model$Cp  # number of parameters - respective Cp value
    chosen.s <- head(which(cp.diff>0), 1) # and take first element which is positive
    return(chosen.s)
})
...
lar.optimal.s <- lapply(lar.models, function(lar.model) {
